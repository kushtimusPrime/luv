...
from fcvision.utils.mask_utils import smooth_mask,get_mask
...
colors = {
	...
	'blue': [(np.array([95, 100, 150]), np.array([130, 255, 255]))]
}
# DATA_DIR = 'data/cable_blue_painted_images'
# colors = {
# 	'green': [(np.array([80, 100, 80]), np.array([110, 255, 220]))],
...
def create_masks():
	...
	for f in uvimgs:
		...
		mask = get_mask(uv_im, colors['blue'] + colors['green'], plot=False,smooth=True)
		save_file=f"{DATA_DIR}/{mask_name}"
		vis=get_mask_vis(uv_im,mask)
